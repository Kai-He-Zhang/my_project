...
for q = 1:length(file_name)
    clearvars -except file_name file q txt_name fp
    disp(file_name{q});
    origin_img = imread(file_name{q});
    gray_img = rgb2gray(origin_img);
    p1_filter = [-1 0 1; -1 0 1; -1 0 1];
    img2 = imfilter(gray_img, p1_filter);
    p2_filter = [1 0 -1; 1 0 -1; 1 0 -1];
    ...
    for i = 1:j
        ...
        x1 = valid_bbox(2) - 10;
        x2 = valid_bbox(2) + uint64(valid_bbox(4)) + 10;
        y1 = valid_bbox(1) - 10;
        y2 = valid_bbox(1) + uint64(valid_bbox(3)) + 10;
        if x1 == 0
            x1 = 1;
        end
        if y1 == 0
        ...
        seg_img = imopen(seg_img, [1 1 1; 1 1 1; 1 1 1]);
        ...
        valid_bboxs = tmp_bbox(-5 <= tmp_bbox(:, 4) & tmp_bbox(:, 4) <= 5, :);
